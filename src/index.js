module.exports = function toReadable (number) {
    ...
    function convert_teens(num) {
        num < 20 ? return get_unit(num)

        let teens = Math.floor(num/10)*10;
        let unit = num % 10;
        (unit) ? return get_teens(teens) + ' ' + get_unit(unit) : return get_teens(teens)
     }
    function convert_hundred(num) {
        let hundred = Math.floor(num/100)*100;
        let teens = num % 100;
        (teens) ? return get_hundred(hundred) + ' ' + convert_teens(teens) : return get_hundred(hundred)
    }
